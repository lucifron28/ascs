...
import { Eye, BarChart3, AlertCircle } from 'lucide-react';
...
export default function DeanReportsPage() {
  ...
  const loadFilterOptions = async () => {
    try {
      const res = await fetchReportFilterOptionsAction('dean');
  ...
  const loadReportData = async (activeFilters: ReportFilters) => {
    ...
    setError(null);
    try {
      const res = await fetchDeanReportSummaryAction(activeFilters);
      if (res.success && res.summary) {
        setSummary(res.summary);
      } else {
        setError(res.error || 'Failed to load Dean clearance report.');
        setSummary(null);
      }
    } catch (err: unknown) {
  ...
  const deanNavLinks = [
    { label: 'Oversight', href: '/dean/dashboard', icon: <Eye className="w-3.5 h-3.5" /> },
    { label: 'Reports', href: '/dean/reports', icon: <BarChart3 className="w-3.5 h-3.5" />, active: true },
  ];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-black tracking-tight text-base-content flex items-center gap-2">
              <BarChart3 className="w-7 h-7 text-primary shrink-0" /> Academic Clearance Reports (Dean Oversight)
            </h1>
            <p className="text-base-content/70 text-xs md:text-sm mt-1">
              Academic clearance progress, requirement bottlenecks, and program breakdowns for Dean oversight.
            </p>
          </div>

          {summary && (
            <div className="shrink-0">
              <CsvExportButton scope="dean" filters={filters} />
            </div>
          )}
        </div>
