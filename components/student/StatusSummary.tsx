...
import { Award, CreditCard, Check, AlertCircle, AlertTriangle } from 'lucide-react';
...
export default function StatusSummary({ application, financial }: StatusSummaryProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div className={`card border shadow-2xl p-6 rounded-2xl flex flex-row items-start gap-4 transition-all ${
        ...
        <div className="space-y-1 flex-1">
          ...
          <h2 className="text-2xl font-black tracking-tight">
            {financial?.status === 'paid' ? 'Paid / Settled' : 'Unpaid Balance'}
          </h2>
          <p className="text-xs text-slate-300 leading-relaxed">
            {financial?.status === 'paid'
              ? 'All financial records are clear. No accountability flags found.'
              : financial?.notes || 'Accounts has unsettled dues. Clearance blocks remain active.'}
          </p>
