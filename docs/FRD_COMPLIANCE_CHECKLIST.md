...
| **Password reset & mandatory change** | `Implemented` | `resetUserTemporaryPasswordAction` issues cryptographically secure temporary passwords, revokes refresh tokens, preserves custom claims, and sets `mustChangePassword: true` with fallback sync and warning output. Mandatory password change is non-bypassable: profile retrieval is enabled via `getAuthenticatedUserForPasswordChange()`, session verification uses Edge-compatible X.509 certificates in `proxy.ts`, and normal actions are blocked by `getAuthenticatedUser()`. Completion requires server-verified credentials via `/api/auth/change-password` with claim rollback and safe failure recovery. |
